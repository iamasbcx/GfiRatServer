...
BEGIN_MESSAGE_MAP(CFileManagerDlg, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_REMOTE_DESKTOP, &CFileManagerDlg::OnBnClickedRemoteDesktop)
	ON_COMMAND(IDM_COMPRESS, &CFileManagerDlg::OnCompress)
	ON_COMMAND(IDM_UNCOMPRESS, &CFileManagerDlg::OnUncompress)
	ON_BN_CLICKED(IDC_LOCAL_DESKTOP, &CFileManagerDlg::OnBnClickedLocalDesktop)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST_LOCAL, &CFileManagerDlg::OnColumnclickListLocal)
	ON_NOTIFY(LVN_COLUMNCLICK, IDC_LIST_REMOTE, &CFileManagerDlg::OnColumnclickListRemote)
END_MESSAGE_MAP()
...
#define LOCAL_TOOLBAR                     150
#define REMOTE_TOOLBAR                    151
BOOL CFileManagerDlg::OnInitDialog() 
{
    ...
    if (!m_wndToolBar_Local.Create(this, WS_CHILD |
		...
		||!m_wndToolBar_Local.LoadToolBar(ID_LOCAL_TOOLBAR))
    {
    ...
    m_ProgressCtrl->SetPos(1);                 //设置进度条当前位置
...
void CFileManagerDlg::OnSize(UINT nType, int cx, int cy) 
{
	...
	GetDlgItem(IDC_REMOTE_DESKTOP)->MoveWindow(53+477, (cy / 2) +1, 66, 20);
...
{
	// todo：在​​此处添加您的消息处理程序代码和/或呼叫默认值
//此例程是拖放操作的末尾。
//释放按钮时，我们要删除该项目。
//我们需要做一些事情来清理和解
//最终确定下降：
// 1）释放鼠标捕获
// 2）将M b拖动为false以表示我们没有拖动
// 3）实际上删除项目（我们调用一个单独的函数来做到这一点）
...
void CFileManagerDlg::OnRclickListLocal(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	pM->DeleteMenu(11, MF_BYPOSITION);
	pM->DeleteMenu(10, MF_BYPOSITION);
	pM->DeleteMenu(9, MF_BYPOSITION);
	pM->DeleteMenu(8, MF_BYPOSITION);
...
void CFileManagerDlg::OnRclickListRemote(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	CPoint	p;
	GetCursorPos(&p);
	pM->DeleteMenu(ID_MENUITEM_OPEN_FILEPATH, MF_BYCOMMAND);
	...
	if (pListCtrl->GetSelectedCount() == 1)
	{
		...
		if (pListCtrl->GetItemData(pListCtrl->GetSelectionMark()) == 1)
		{
			...
			pM->EnableMenuItem(8, MF_BYPOSITION | MF_GRAYED);
		}
			
		else
		{
			pM->EnableMenuItem(nRemoteOpenMenuIndex, MF_BYPOSITION | MF_ENABLED);
			pM->EnableMenuItem(8, MF_BYPOSITION | MF_ENABLED);
		}
...
void CFileManagerDlg::OnBnClickedRemoteDesktop()
{
	// TODO: 在此添加控件通知处理程序代码
...
void CFileManagerDlg::OnBnClickedLocalDesktop()
{
	// TODO: 在此添加控件通知处理程序代码
		// TODO: 在此添加控件通知处理程序代码
	...
	char m_strMyDesktopPath[MAX_PATH] = {};
	SHGetFolderPath(NULL, CSIDL_DESKTOPDIRECTORY, hTokenAcsi, SHGFP_TYPE_CURRENT, (char*)&m_strMyDesktopPath);
	CloseHandle(hTokenAcsi);
	//	RevertToSelf();

	m_Local_Directory_ComboBox.SetWindowText(m_strMyDesktopPath);
	m_list_local.SetSelectionMark(-1);
	FixedLocalFileList();
}

HANDLE CFileManagerDlg::ImpersonateLoggedOnUserWrapper()
{
	typedef DWORD
	(WINAPI
...
static int CALLBACK CompareFunc(LPARAM   lParam1, LPARAM   lParam2, LPARAM   lParamSort)
{
	...
	int iItem1 = pListCtrl->FindItem(&findInfo,2);
	findInfo.lParam = lParam2;
	int iItem2 = pListCtrl->FindItem(&findInfo, 2);
...
static int CALLBACK CompareFunc2(LPARAM   lParam1, LPARAM   lParam2, LPARAM   lParamSort)
{
	...
	CString         strItem1, strItem2;
	strItem1 = pListCtrl->GetItemText(iItem1, dwSelColID2);
	strItem2 = pListCtrl->GetItemText(iItem2, dwSelColID2);
	if (bASC2)//升序
		return strcmp(strItem1, strItem2);
	return   strcmp(strItem2, strItem1);
}

void CFileManagerDlg::OnColumnclickListLocal(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMLISTVIEW pNMLV = reinterpret_cast<LPNMLISTVIEW>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	if (dwSelColID2 != pNMLV->iSubItem)//选择的不是相同列
	{

		dwSelColID2 = pNMLV->iSubItem;//得到新的列号
		bASC2 = TRUE;  //第一次选择是升序排列
	}
	else
	{
		bASC2 = !bASC2;//改变排序规则
	}
	m_list_local.SortItems(CompareFunc2, (LPARAM)(&m_list_local));
	*pResult = 0;
}


void CFileManagerDlg::OnColumnclickListRemote(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMLISTVIEW pNMLV = reinterpret_cast<LPNMLISTVIEW>(pNMHDR);
	// TODO: 在此添加控件通知处理程序代码
	...
	m_list_remote.SortItems(CompareFunc, (LPARAM)(&m_list_remote));
	*pResult = 0;
